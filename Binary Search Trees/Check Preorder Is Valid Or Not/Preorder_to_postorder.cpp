...
Node* createBST(int pre[],int *pos,int key,int l,int h,int size)
{
    if(*pos>=size)
        return NULL;
    Node *root=NULL;
    if(key>l && key<h)
    {
        ...
        *pos=*pos+1;
        if(*pos<size)
            root->left=createBST(pre,pos,pre[*pos],l,key,size);
        if(*pos<size)
            root->right=createBST(pre,pos,pre[*pos],key,h,size);
    }
    return root;
}
...
Node* constructTree(int pre[], int size) {
    //code here
    int pos=0;
    return createBST(pre,&pos,pre[0],INT_MIN,INT_MAX,size);
}
