...
int getMinPartition(int arr[],int l,int r)
{
    int n=r-l+1,pos=rand()%n;
    swap(arr[l+pos],arr[r]);
    return minPartition(arr,l,r); 
}

int getMaxPartition(int arr[],int l,int r)
{
    int n=r-l+1,pos=rand()%n;
    swap(arr[l+pos],arr[r]);
    return maxPartition(arr,l,r); 
}
...
int ksmallest(int arr[],int l,int r,int k,int n)
{
    if(k>0 && k<=r-l+1)
    {
        int index=getMinPartition(arr,l,r);
        if(index-l==k-1)
            return arr[index];
        else if(index-l<k-1)
...
int klargest(int arr[],int l,int r,int k,int n)
{
    if(k>0 && k<=r-l+1)
    {
        int index=getMaxPartition(arr,l,r);
        if(index-l==k-1)
            return arr[index];
        else if(index-l<k-1)
